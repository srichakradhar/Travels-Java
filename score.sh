...
randData=$(strings web/logs | grep -o 'randData/[a-z/@.0-9\-]*' | cut -d '/' -f 2-)

registerData=$(strings web/logs | grep -o 'regData/[a-z/@.0-9\-]*' | cut -d '/' -f 2-)

logincaptchaData=$(strings web/logs | grep -o 'loginData/[a-z/@.0-9\-]*' | cut -d '/' -f 4 | cut -d '.' -f 1)
logincaptchaImageData=$(strings web/logs | grep -o 'loginData/[a-z/@.0-9\-]*' | cut -d '/' -f 5 | cut -d '.' -f 1)
dragPing=$(strings web/logs | grep -o 'Drag/[a-z/@.0-9\-]*')
echo $dragPing
